  ...
  while (oscillationDetected && recursionCount < maxOscillationIterations) {
    ...
    for (let step = 1; step <= projectionSteps; step++) {
      ...
      for (let d = 0; d < effectivePrimes.length; d++) {
        ...
        const cosineArg = (n_new - 1) * 2 * Math.PI / 12 * phi_d;
        const tetratedValue = tetration(3, TETRATION_DEPTH);
        const tetrationScale = Math.log(tetratedValue) / (Math.log(3) * TETRATION_DEPTH);
        const scaledExponent = exponent * tetrationScale;
        ...
        for (const k of kValues) {
          const lambda = d % 3;
          // Recursive self-similar calculation with 3 layers
          const recursiveL = recursiveLatticeLayer(n_new, d, k, lambda, 0, 2, effectivePrimes, historicalPrices);
          lSum += recursiveL;
        }
        
        // P function with tetration depth 31
        const theta = calculateTheta(n_new, step, 0, 144000, 0);
        ...
        const combinedFactor = (zValue * 0.4 + lSum * 0.3 + pValue * 0.3);
        ...
        // Ensure continuity from last price
        const projection = lastPrice * Math.pow(1 + stepChangeFactor, step);
        
        // Apply normalization
        const normalizationFactor = calculateNormalizationFactor(historicalPrices, projection);
        const normalizedProjection = lastPrice + (projection - lastPrice) * normalizationFactor;
      ...
      let baseProjection = totalWeight > 0 ? weightedSum / totalWeight : lastPrice;
      ...
      if (model.lockedPoints && Array.isArray(model.lockedPoints) && model.lockedPoints.length > 0) {
        ...
        const lockedInfluence = recentLocked.reduce((sum, point) => {
          const distance = Math.abs(n - point.index);
      ...
      if (step === 1) {
        ...
        const continuityFactor = 0.95; // Allow 5% max deviation
        ...
        // Subsequent projections should be smooth
        const prevProjection = projections[step - 2];
        const maxStepChange = prevProjection * 0.05; // Max 5% change per step
      ...
      let projectedPrice = baseProjection * Math.pow(1 + normalizedTrend, step);
